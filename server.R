...
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ Getting the tidycensus data $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$#
    ...
    library(stringr)
    state_name <- as.character(city_table[which(city_table$city == tolower(trimws(input$location_box))), c("state_name")])
    
    ## Getting the data from the tidycensus API
    # Assigning the IDs repective to the demographic features into a dataframe
    demographic_tbl <- data.frame(demo_name=c("Total median age","Total Median age of Males","Gross Median Rent","Mortgage","Not interested"), id=c("B01002_001","B01002_002","B25031_001","B25097_002","B25097_003"))
  ...
  observeEvent(input$search_button, {  
    ...
    compress <- jsonlite::flatten(business_data[[1]]) %>% select(-id, -is_closed, -categories, -location.display_address, -categories, -transactions, -coordinates.latitude, -coordinates.longitude, -distance, -phone)
